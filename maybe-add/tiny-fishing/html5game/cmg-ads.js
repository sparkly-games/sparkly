var version = "2.1.2-h5-ads-fallback-on-playwire-ad-error";
...
function cmgRewardsAdBreakH5() {   //Google H5 ads using GAM 
  ...
  googletag.cmd.push(() => {
    ...
    if (rewardedSlot) {
      ...
      googletag.pubads().addEventListener("rewardedSlotReady", (event) => {
        console.log("H5:::Rewarded ad slot is ready.");
        event.makeRewardedVisible(); // âœ… Show the ad
      });

      googletag.pubads().addEventListener("rewardedSlotClosed", () => {
        console.log("Rewarded ad has been closed.");
        if (rewardedSlot) googletag.destroySlots([rewardedSlot]); // âœ… Safe to destroy now
        onAllAdsCompleted();
      });
